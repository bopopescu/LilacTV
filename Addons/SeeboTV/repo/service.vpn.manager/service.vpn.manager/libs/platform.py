...
def getPlatform():
    ...
    if sys.platform == "linux" or sys.platform == "linux2":
        if build_number == 15 and getAddonPath(True, "").startswith("/storage/.xbmc/"):
...
def useSudo():
    ...
    if getPlatform() == platforms.LINUX:
        ...
        if not getAddonPath(True, "").startswith("/storage/.xbmc/"): return True
    return False
...
def checkPidofCommand(addon):
    ...
    if p == platforms.RPI or p == platforms.LINUX:
        # Issue Linux command
        command = getPidofPath() + " xbmc.bin > " + getTestFilePath() + " 2>&1 &"
        if useSudo() : command = "sudo " + command
...
def getLogPath():    
    return xbmc.translatePath("special://logpath/xbmc.log")
