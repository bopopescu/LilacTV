...
ROOT_URL   = "https://www.gomlab.com/subtitle"
...
def Search( item ):
    ...
    for sub in subtitles_list:
        ...
        url = "plugin://%s/?action=download&link=%s&ID=%s&filename=%s&format=%s" % (__scriptid__,urllib.quote(sub["link"]),sub["ID"],sub["filename"],'ass' if convertASS else sub["format"])
...
def SearchSubtitles(q_url):
    ...
    subtitles = []
    for row in soup.find("table",{"class":"tbl tbl_board transform"}).findAll("tr")[1:]:
        a_node = row.find("a")
        if a_node is None:
            continue
        title = a_node.text        
        url = ROOT_URL + "/" + a_node["href"].replace("view.gom","download.gom")
        langlong  = "Korean"
        langshort = "ko"
        subtitles.append( {
            "link"          : url,
...
def GetSubtitleUrl(page_url):
    ...
    if "a href=\"jamak://gom.gomtv.com" in html:
        ...
        #sp2 = "/download.gom?seq="+intSeq+"&keyword=infinity&preface=kr"
    else:
        return None
    return ROOT_URL+sp2
