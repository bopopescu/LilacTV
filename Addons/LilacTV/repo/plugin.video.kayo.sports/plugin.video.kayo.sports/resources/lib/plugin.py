...
def home(**kwargs):
    ...
    if not api.logged_in:
        ...
        text ="""
        1. 이 애드온은 유료로 제공되는 스포츠 Live / VOD 서비스 입니다.

        2. PC나 모바일에서 https://kayosports.com.au/ 사이트로 이동합니다.

        3. 35$/month 프리미엄 계정(14일 무료)을 하나 만듭니다.

        4. 라일락TV에서 Kayo Sports 애드온을 클릭합니다.

        5. 위에서 만든 계정으로 Login 합니다.

        6. 아무 컨텐츠나 실행해서 정상적으로 동작하는지 확인합니다.

        7. 다시 https://kayosports.com.au/ 사이트로 이동합니다.

        8. 사이트 오른쪽 상단의 "리스트 메뉴" > "My Account" 항목으로 이동합니다.

        9. 아래로 스크롤다운 해서 맨 아래에 있는 "Cancel subscription" 선택합니다.

        10. 다시 라일락TV에서 정상 동작하는 확인 합니다. Enjoy!!!
        """

        dialog.textviewer('    [Kayo Sports] 애드온 무료 사용법', text)
...
@plugin.route()
def panel(id, sport=None, select=None, **kwargs):
