...
def artPath():
    ...
    if theme in ['-', '']:
        return
    elif condVisibility('System.HasAddon(script.tempest.artwork)'):
        return os.path.join(xbmcaddon.Addon(
            'script.tempest.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting(
        'appearance.1').lower() if condVisibility(
        'System.HasAddon(script.tempest.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.tempest.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.tempest.metadata)'):
        return os.path.join(xbmcaddon.Addon(
            'script.tempest.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
