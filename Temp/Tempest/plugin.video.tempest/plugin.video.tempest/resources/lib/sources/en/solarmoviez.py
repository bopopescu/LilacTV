...
class source:
    def __init__(self):
        self.priority = 1
        self.language = ['en']
        self.domains = ['solarmoviez.ru','https://www0.solarmovies.co', 'solarmovie.to']
    ...
    def searchShow(self, title, season, aliases, headers):
        try:
            ...
            url = self.base_link + self.search_link % search
            r = self.s.get(url, headers=headers).content
            r = client.parseDOM(r, 'div', attrs={'class': 'ml-item'})
            r = zip(client.parseDOM(r, 'a', ret='href'), client.parseDOM(r, 'a', ret='title'))
            r = [(i[0], i[1], re.findall('(.*?)\s+-\s+Season\s+(\d)', i[1])) for i in r]
            r = [(i[0], i[1], i[2][0]) for i in r if len(i[2]) > 0]
            url = [i[0] for i in r if self.matchAlias(i[2][0], aliases)][0]
            return url
        except:
            return
    ...
    def searchMovie(self, title, year, aliases, headers):
        try:
            ...
            for i in r:
                try:
                    info = self.s.get(urlparse.urljoin(self.base_link, self.info_link % i[2]), headers=headers).content
